...
class DocsController extends BaseController {
  ...
  void fetchDocs() async {
    try { 
      ...
      clearError();
      final response = await apiService.getDocs();
      topics.value = response.topics?.topicList?.topics ?? [];    
      l.d('获取文档列表成功: ${topics.length} 条数据');
    } catch (e, s) {
      l.e('获取文档列表失败: $e $s');
      setError('获取文档列表失败');
    } finally {
  ...
  Future<void> onRefresh() async {
    try {
      ...
      // 更新用户缓存
      _userCache.updateUsers(response.topics?.users ?? []);

      topics.value = response.topics?.topicList?.topics ?? [];
      hasMore.value = response.topics?.topicList?.moreTopicsUrl != null;
      currentPage.value = 1;
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      final response = await apiService.getDocs(nextPage);

      // 更新用户缓存
      _userCache.updateUsers(response.topics?.users ?? []);

      topics.addAll(response.topics?.topicList?.topics ?? []);
      hasMore.value = response.topics?.topicList?.moreTopicsUrl != null;
